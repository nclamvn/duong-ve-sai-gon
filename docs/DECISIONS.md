...
| D-067 | Chủ nhà → Thợ | Tay FP "như bộ xương/zombie" trên Mac: ép (k,k,1) của D-065 kéo dài cẳng tay hướng vào màn hình → **thay bằng lớp viewmodel riêng** (`vmScene` + `vmCamera` FOV = 2·atan(k·tan(fov/2)), pass riêng mix theo alpha sau AO/SSR, trước bloom; `post=off` render 2 lần với clearDepth); FX nòng/vỏ đạn chiếu về camera chính; bỏ tint xanh xám trên tay | Ảnh Mac 16:59 2026-09-03 |
| D-068 | Chủ nhà | 2026-09-03 chiều: "hiểu lại yêu cầu — tạo một màn chơi hoành tráng ban ngày khác" → Blueprint G0.6 coi như APPROVED, làm TIP-019 ngay trong phiên; arena G0 giữ cho bench/E2E (`?level=arena`), chơi mặc định = Phố Vạn Hải | TIP-019 |
| D-069 | Thợ (L1) | Level = JSON + builder trả `ArenaData` (cùng hợp đồng arena) → physics/nav/AI/mission không đổi; nhà ống/chướng ngại/xe cháy procedural gộp theo material, prop glTF instanced theo primitive, cột điện procedural (model Poly Haven 85k tri) — draw 2 500 → ~620, tri 6.2 M → 2.4–3.1 M | TIP-019 |
| D-070 | Thợ (L1) | Ban ngày: sun + CSMShadowNode 2 cascade (3 cascade = ×4 draw), HDRI 2K làm nền + IBL, prop < 0.8 m không đổ bóng; FX khói/lửa/tàn lửa/bụi TSL instanced billboard (vertexNode clip-space, không lấy phần tử ma trận), lửa ≤ 6 PointLight | TIP-019 |
| D-071 | Thợ (L1) | Payload asset 60 → ≤ 150 MB theo `initial_payload_mb` budget (level ngày +75 MB CC0); fetch-assets giữ entry Mixamo/Sketchfab (bug ghi đè manifest đã sửa); manifest id cho phép chữ hoa (Poly Haven `Barrel_01`) | TIP-019 |
| D-072 | Thợ (L1) | Loại 2 model Sketchfab CC-BY vì là asset rip từ game (PRD §9.2): BTR-80 (42manako, "Ripped from Squad") và Mi-8 (tnikita, material `bo2_*` = CoD Black Ops 2) → thay BTR-70 (veightyfive) và Mi-24 Hind (Duane's Mind); quy tắc: đọc mô tả/tag/tên material trước khi tải, nhãn CC-BY trên Sketchfab không đủ | TIP-021 |
| D-073 | Thợ (L1) | Model thật thay procedural bằng **dữ liệu** (`BarricadeDef.model/size`), không đổi builder/AI: collider + cover lấy từ `size` sau chuẩn hoá (dài +x, đáy y=0), thiếu model → procedural cũ; skin tầm thường của Sketchfab được nướng lúc convert (xe buýt trôi 1,5 m nếu không); join primitive theo material (bán tải 26 → 8, buýt 16 → 4 draw) | TIP-021 |
| D-074 | Thợ (L1) | Payload: bỏ `modular_electricity_poles` (6 MB, không dùng từ TIP-019) + xe cháy texture 512 → manifest 145 MB ≤ 150 MB budget; không sửa budget. Draw sandbox 650–740 (TIP-019: 620–683) — nợ TIP-026 | TIP-021 |
| D-075 | Chủ nhà → Thợ | Mac e203f1e: "không thấy nhà cửa đâu chỉ trời mây" — builder level (TIP-019) **thiếu collider mặt đất** (arena có `floor`, level mới không) → rơi xuyên đất. Probe sandbox chỉ chụp 2 frame, bot đi navmesh, E2E dùng `level=arena` nên không lộ. Sửa: collider floor + vỉa hè 15 cm (khớp hình); thêm E2E `level-pho.spec.ts` (đứng 3 s, chạy 6 s tới chốt A, lên vỉa hè) chạy trong `npm run ci`. Quy tắc: level mới phải có E2E vật lý người chơi, không chỉ ảnh | Hotfix e203f1e → TIP-021b |
