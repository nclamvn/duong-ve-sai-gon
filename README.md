...
`http://127.0.0.1:5173/?autostart=1` (mặc định level M1 Trường Sơn — DV-044; `?level=arena|pho` cho test; telemetry chỉ `&overlay=1` hoặc F3). Nếu "ERR_CONNECTION_REFUSED": dev server chưa chạy hoặc đã tắt — chạy lại `npm run dev` trong Terminal và để nguyên.
...
→ `nav.bin` (+ manifest); thêm `--level content/levels/truong-son-a.level.json` để bake **obstacle thân cây** (bot đi vòng cây, DV-040); `node scripts/find-clearing.mjs <x> <z> 300 18 0.08` tìm bãi trống tự nhiên trong rừng scatter (điểm treo đổ quân/LZ, DV-042). Bầu trời (TIP-D-SKY): khối `airTraffic` của level → `engine/sky` (lịch bay seeded, model `air_*` mũi +x, node tên `rotor|blade|prop` quay; mission kích hoạt bằng action `sky_trigger`). Bản đồ giấy cho minimap/bản đồ chiến thuật (TIP-UX02): `node scripts/terrain-bake.mjs --map --id truong-son-a --level content/levels/truong-son-a.level.json` → `map.png`. Mission M1: `content/missions/truong-son-a.mission.json` + `truong-son-a.dialogue.json` (đồng đội `archetype: squad` + `names`, thám báo `archetype: recon`; `src/game/ai/squadmates.ts`). HUD: `src/ui/hud.ts` + `hud.css` (font OFL `public/assets/fonts/`), phím M bản đồ, N minimap. Level: `content/levels/truong-son-a.level.json` (schema `terrain-level.schema.json`; spawn/waypoint theo x/z, y từ terrain). Nhân vật Mixamo (Swat Guy + 8 clip,
